...
|  題號 | 題目核心                                                        | 選錯                                                                                              | 正解                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 弱點分類                                                                                                                                                                                   |
| --: | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  Q2 | 何種控制會影響 egress monitoring solution 功能                       | Firewall                                                                                        | Encrypti                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | encryption 對監控可視性的影響                                                                                                                                                                   |
|  Q4 | 哪個不是 enhanced authentication 常見手段                           | Dynamic end-user knowledge(動態終端使用者知識-系統不問你「你媽媽的本名（靜態知識）」，而是問你「你上一筆信用卡的消費金額是多少？」或發送一次性密碼 (OTP)。) | Variable keystrokes(變動的擊鍵,詞的意思是「每次打字都不一樣、充滿變數」。如果一個人的擊鍵模式是「變動的」，系統就無法建立可靠的基準範本。**系統沒有辦法拿一個「每次都在變」的東西來確認「你就是你」。** 因此，它極度脆弱 (weak)，不能作為 enhanced authenticatio                                                                                                                                                                                                                                                                                                                                                                                                            | authentication factor / biometric 行為特徵<br>*在 CCSP/CISSP 中，看到 "Dynamic" 通常是好事（代表難以重播攻擊），看到 "Behavioral Biometric" 也是好事；但如果看到特徵是 "Variable" (不穩定的變數)、"Inconsistent" (不一致)，那就絕對不能用來做身分認證* |
|  Q6 | Public cloud data center control governance 由誰制定            | Regulator(s)                                                                                    | Cloud prov                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | provider governance / responsibility boundary                                                                                                                                          |
|  Q7 | VM configuration management tool 應包含什麼                      | Anti-tampering mechanisms                                                                       | Log file gener                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | CM / logging / incident response                                                                                                                                                       |
| Q10 | Cloud data center campus physical access point 應包含哪些，except | Badging procedure                                                                               | Mantrap stru                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | physical entrance vs sensitive-area control                                                                                                                                            |
| Q13 | 哪組 control 可互相增強保護                                          | Asset inventories/personnel training                                                       Data dispersion(資料分散)/encryption<br>-- **加密 (Encryption)：** 把資料變成亂碼。<br>    <br>    - _單獨使用的弱點：_ 如果駭客成功竊取了「整包」加密檔案，他可以把檔案帶回家，用超級電腦慢慢進行離線暴力破解 (Offline Brute-force)。<br>        <br>- **資料分散 (Data Dispersion)：** 像是雲端常用的 Bit Splitting 或 Erasure Coding 技術，把一個檔案「切成好幾塊碎片」，分別存放在不同的資料中心或可用區 (Availability Zones)。<br>    <br>    - _單獨使用的弱點：_ 如果沒有加密，駭客只要攻破其中一個資料中心，雖然他拿不到完整檔案，但他手上的那塊「明文碎片」可能剛好包含了客戶的信用卡號或身分證字號，依然會造成資料外洩。<br>	- **協同效應 (The Synergy) ** <br>	當把這兩者結合（**先加密，再切碎分散**），防禦力會呈指數型上升。 想像一下，駭客今天費盡千辛萬苦攻破了雲端供應商的 Data Center A，他偷走的東西是什麼？ 答案是：**「不完整的亂碼碎片」**。 洩。<br>	 | layered defense / control synergy                                                                                                                                                      |
| Q17 | Cloud penetration test 為何不是最佳攻擊模擬                           | Regulator involvement changes attack surface                                                    | Advanced notice removes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | cloud pentest authorization reality                                                                                                                                                    |
| Q20 | BC/DR plan/policy 應包含哪些，except                              | Tasking responsible office                                                                      | Copies of laws/regulations/                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | BC/DR policy content vs referenced governance docs                                                                                                                                     |
| Q24 | Data center redundancy effort 中最大 human safety threat       | Emergency egress                                                                                |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | facility redundancy / generator fuel safety                                                                                                                                            |
