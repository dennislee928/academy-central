...
|  Q4 | 哪個不是 enhanced authentication 常見手段                           | Dynamic end-user knowledge                   | Variable keystrokes(變動的擊鍵,詞的意思是「每次打字都不一樣、充滿變數」。如果一個人的擊鍵模式是「變動的」，系統就無法建立可靠的基準範本。**系統沒有辦法拿一個「每次都在變」的東西來確認「你就是你」。** 因此，它極度脆弱 (weak)，不能作為 enhanced authentication。) | authentication factor / biometric 行為特徵             |
