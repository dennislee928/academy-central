...
「誰負責哪一層？」  
「哪個選項最符合 governance？」  
「題目是在問控制、可見性、可攜性，還是可用性？」

---

## 二、錯題主軸 1：Hybrid vs Community vs Private
...
## 四、錯題主軸 3：Insider threat + shared responsibility
...
SaaS 會減少應用更新與維護負擔。檔案中 Q2 指出 SaaS 相比 PaaS/IaaS，客戶較少處理 application maintenance 與 patch coordination。

但 SaaS 不代表全部責任都交給服務商。  
IT 部門會轉向：
...
### CSA CCM vs ISO 27017

Q3 的 CSA CCM 是用來評估 cloud security posture，且可對應 ISO/IEC 27001、PCI DSS、NIST 等標準。

Q4 的 ISO/IEC 27017 是 cloud-specific security control guidance。
...
Q5 正解是採用 **cloud-agnostic architectures and standard data formats**。

記法：

**減少 lock-in = 標準化、可攜性、避免過度依賴 provider-specific features**

---

### Containerization

Q7 指出 containers 可提升 portability 與 security，但需要正確設定。
...
Q8 的重點是 serverless 讓底層 infrastructure visibility 降低，因此較難量化安全收益。

記法：

**Serverless = less infrastructure management, less infrastructure visibility**

---

### Rapid elasticity

Q10 說 rapid elasticity 最適合 **stateless、loosely coupled** 的架構。

所以不要選：

「不需要 capacity planning」  
因為雲端仍需要 capacity planning，只是方式變成自動化、監控與成本治理。
